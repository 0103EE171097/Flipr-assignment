...
function TableView(props) {
  ...
  let line = {
    ...
    bottom:
      deliverys.length > 0
        ? 170 - deliverys.length * 35
        : 170 - delivery.length * 32,
        ...
        <div className="col-4 left-table">
          <div className="border shadow-sm">
            <img
              ...
              width="35"
              height="35"
              className="rounded-circle img-background"
              alt=""
              loading="lazy"
            />
            ...
            <img
              ...
              width="35"
              height="35"
              className="rounded-circle home-background"
              alt=""
              loading="lazy"
            />
          </div>
              ...
              <tbody>
                {apiData.map((item, index) => (
                  <tr
                    ...
                    onClick={() => (item.scan ? setDelivery(item.scan) : [])}
                  >
                    <td>#{item.awbno}</td>
                    <td>{item.carrier}</td>
                    <td>{item.from}</td>
                    ...
                    <td>
                      {item.extra_fields
                        ? moment
                            .utc(item.extra_fields.expected_delivery_date)
                            .local()
                            .format("DD/MM/YYYY")
                        : ""}
                    </td>
